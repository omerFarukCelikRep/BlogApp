...
{
    public static Result Success(T? data, string message = "", int statusCode = 200) =>
        new Result<T>(data, true, string.Empty, statusCode, Error.None);

    public static Result Failed(T? data, string message, int statusCode) =>
        new Result<T>(data, false, message, statusCode, new Error(string.Empty, message));

    public static Result Failed(T? data, string message, int statusCode, Error error) =>
        new Result<T>(data, false, message, statusCode, error);
}